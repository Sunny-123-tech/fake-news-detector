...
News text: {request.text}"""
    response = client.chat.completions.create(model="llama-3.3-70b-versatile", messages=[{"role": "user", "content": prompt}], max_tokens=500, temperature=0.3)
...
async def analyze_image(file: UploadFile = File(...)):
    ...
    prompt = f"An image named '{file.filename}' ({size} bytes) was uploaded for fake news detection. ELA score is {ela_score}. Give a 2-sentence analysis of whether this image might be manipulated or authentic."
    ...
    return {"filename": file.filename, "verdict": verdict, "confidence": round(1 - deepfake_prob, 2), "ela_score": ela_score, "deepfake_probability": deepfake_prob, "explanation": explanation}
...
{{"verdict": "AUTHENTIC" or "POSSIBLY MANIPULATED" or "DEEPFAKE SUSPECTED", "confidence": 0.0-1.0, "explanation": "2-3 sentences about video authenticity analysis"}}"""
    response = client.chat.completions.create(model="llama-3.3-70b-versatile", messages=[{"role": "user", "content": prompt}], max_tokens=300, temperature=0.3)
    raw = response.choices[0].message.content
    match = re.search(r'\{.*\}', raw, re.DOTALL)
    if match:
        data = json.loads(match.group())
        return {"filename": file.filename, **data}
    return {"filename": file.filename, "verdict": "PROCESSING", "confidence": 0.0, "explanation": raw}
